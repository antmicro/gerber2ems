...
class Postprocesor:
    ...
    def process_data(self):
        """
        Calculates all needed parameters for further processing.
        Should be called after all ports are added
        """
        logger.info(
    ...
    def get_impedance(self, port: int) -> Union[np.ndarray, None]:
        """Returns specified port impedance"""
        if port >= self.count:
            logger.error("Port no. %d doesn't exist", port)
            return None
        elif self.is_valid(self.impedances[port]):
    ...
    def render_s_params(self):
        """Renders all S parameter plots to files"""
    ...
    def render_smith(self):
        """Renders port reflection smithcharts to files"""
    ...
    def is_valid(array: np.ndarray):
        """Checks if array doesn't have any NaN's"""
